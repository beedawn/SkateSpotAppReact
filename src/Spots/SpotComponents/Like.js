...
export default function Like(props) {
  ...
  const userDb = userArray.filter((person) => {
    return user.photoURL === person.id;
  });
  ...
  const editLike = async () => {
    ...
    if (userDb[0].likedSpots.includes(spot.id)) {
      const newArray = userDb[0].likedSpots.filter((item) => {
        return item !== spot.id;
      });
      const payload = {
        ...userDb[0],
  ...
  return !isLoaded ? (
    ...
    <>
      <div>
        {" "}
        {userDb[0].likedSpots.includes(spot.id) ? (
          <>
            <a href="#!" className="like">
              <FaHeart size={70} onClick={determineLike} />
            </a>
          </>
        ) : (
          <>
            <a href="#!" className="like">
              <FaRegHeart size={70} onClick={determineLike} />
            </a>
          </>
        )}
      </div>{" "}
